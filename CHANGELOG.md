...
- The step-level mocha and node:test plugins now pass the runnable to the `steps` iterator as its `step` argument (a `{ name, skip }` object) instead of binding it to `this`. This lets the `steps` callback be an arrow function while still reaching `skip()`; thread the step through the context (`yadda.run(step, { step }, done)`) so step implementations can call `ctx.step.skip()`. `yadda.run`/`yadda` accept the step object directly, matching on its `name`. See https://github.com/acuminous/yadda/issues/352
...
- A `nodetest` plugin (`Yadda.plugins.nodetest`) with `StepLevelPlugin`/`ScenarioLevelPlugin`, wiring feature files into Node's built-in `node:test` runner. Unlike the mocha plugin it returns the `featureFile`/`scenarios`/`steps` helpers rather than assigning globals, since `node:test` has no global injection. A runtime `step.skip()` throws internally so the remaining steps in the scenario are skipped, matching the mocha plugin's behaviour. See https://github.com/acuminous/yadda/issues/335
...
- `MarkdownFeatureParser` and `MarkdownFeatureFileParser`, which parse specifications written as GitHub-flavoured markdown so they render nicely on GitHub. Headings carry the keywords (`# Feature:`, `## Scenario:`), steps are list items, doc-strings are fenced code blocks, and data/example tables are markdown tables. Comments come in two forms, both freeing `#` for headings: `>` blockquotes are visible comments that render as callouts on GitHub, and `<!-- -->` blocks are hidden comments that a viewer does not render. They emit the same events as `FeatureParser` into the shared state machine — extracted into a common `BaseFeatureParser` — so a markdown feature and its `.feature` equivalent parse to identical output. Entities (`&lt;` etc.) are decoded in prose but left verbatim in doc-strings; inline code spans are not yet exempt from decoding.
