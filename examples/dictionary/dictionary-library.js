...
module.exports = (function () {
  ...
  const library = English.localise(new Yadda.ContextParamLibrary(dictionary))
    ...
    .define('Expect $integer to be an integer', (ctx, i, next) => {
      assert.equal(typeof i, 'number');
      assert(i % 1 === 0);
      next();
    })

    .define('Expect $float to be a float', (ctx, f, next) => {
      assert.equal(typeof f, 'number');
      assert(f % 1 !== 0);
      next();
    })

    .define('Expect $date to be a date', (ctx, d, next) => {
      assert.equal(Object.prototype.toString.call(d), '[object Date]');
      next();
    })

    .define('Expect $period to have a quantity of $integer and units of $units', (ctx, period, quantity, units, next) => {
