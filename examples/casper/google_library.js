...
library = new Yadda.Library()

.define("When I open Google's (.+) search page", function(locale) {    
	casper.open("http://www.google." + locale + "/");
})

.define("(?:then|and) the title is (.+)", function(title) {
	casper.test.assertTitle(title, 'title is the one expected');
})

.define("(?:then|and) the (.+) form exists", function(action) {
	casper.test.assertExists('form[action="/' + action + '"]', 'form is found');
})

.define("When I search for (.+)", function(term) {
	casper.fill('form[action="/search"]', {
		q: term
	}, true);
})

.define("(?:then|and) the search for (.+) was made", function(term) {
	var regex = new RegExp('q=' + term)
	casper.test.assertUrlMatch(regex, 'search term has been submitted');
})

.define("(?:then|and) (\\d+) or more results were returned", function(number) {
