...
module.exports = (obj) => {
  ...
  function flatten(items) {
    ...
    const head = flatten(items[0]);
    const tail = flatten(items.slice(1));
  ...
  function each_async(items, iterator, callback) {
    ...
    let index = 0;
    const iterate = () => {
      iterator(items[index], index, (err, result) => {
