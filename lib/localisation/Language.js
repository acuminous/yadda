...
module.exports = function(vocabulary) {
    ...
    this.library = function(dictionary) {
        return this.localise_library(new Library(dictionary));
    };

    this.localise_library = function(library) {
        $(['given', 'when', 'then']).each(function(key) {
            library[key] = function(signatures, fn, ctx) {
                return $(signatures).each(function(signature) {
                    var signature = prefix_signature(vocabulary[key], signature);
