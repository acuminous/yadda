...
// Understands how to index macros. Subclasses override `create_macro` to
// choose which macro type is created.
const BaseLibrary = function (dictionary = new Dictionary()) {
  const macros = $();
  ...
  const define_macro = (signature, fn, macro_context, options) => {
    ...
    macros.push(this.create_macro(signature, dictionary.expand(signature), fn, macro_context, options));
  };
