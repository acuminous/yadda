...
var Interpreter = function(libraries) {
    ...
    this.validate = function(scenario) {
        ...
        if (results.find(by_invalid_step))
            throw new Error('Scenario cannot be interpreted\n' +
    ...
    this.interpret = function(scenario, scenario_context, next) {
        ...
        event_bus.send(EventBus.ON_SCENARIO,
                       {scenario: scenario, ctx: scenario_context.properties});
    ...
    this.interpret_step = function(step, scenario_context, next) {
        ...
        event_bus.send(EventBus.ON_STEP,
                       {step: step, ctx: context.properties});
        this.rank_macros(step).clear_winner().interpret(step,
                                                        context || {}, next);
    };
