...
var Examples = function(scenario) {
    ...
    var SURROUNDING_WHITESPACE_REGEX = /^\s+|\s+$/gm;
    var RIGHT_WHITESPACE_REGEX = /\s+$/gm;
    ...
    function capture_blank(){
        ...
        switch(parsing_mode){
            case 0: // headings
              console.log('capture_blank '+parsing_mode+' '+headings.length);
              if (headings.length>0)
                parsing_mode=2;
              break;
            case 1: // single row mode
            case 2: // multi row mode - new row
              console.log('capture_blank '+parsing_mode);
              break;
            case 3: // multi row mode - row continuation
              ...
              console.log('capture_blank '+parsing_mode+ ' ' + examples.length + JSON.stringify(example));
              console.log(' ---------- ');
    ...
    function capture_headings(event, data) {
        ...
        headings = split(data).collect(function(column) {
            ...
            ident=ident && ident.length && ident[0].length;
            identation.push(ident);
    ...
    function capture_example(event, data) {
        ...
        switch(parsing_mode){
           ...
           case 1: // single row mode
              ...
              console.log('capture_example '+parsing_mode+' '+examples.length+' '+JSON.stringify(example));
           ...
           case  2: // multi row mode - new row
              ...
              examples.push(example);
              console.log('capture_example 2 '+examples.length+' '+JSON.stringify(example));
              parsing_mode=3;
              break;
           case 3: // multi row mode - row continuation
              ...
              fields.each(function(field, index) {
                if (field
                    ...
                    .replace(SURROUNDING_WHITESPACE_REGEX, ''))
                  throw new Error("Identation error");
                example[headings[index]].push(
                  field
                    .substr(identation[index])
                    .replace(RIGHT_WHITESPACE_REGEX, ''));
              });

              console.log('capture_example 3 '+examples.length+' '+JSON.stringify(example));
              break;
        }
    }
