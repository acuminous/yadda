...
var FeatureParser = function(language) {
    ...
    function parse_line(line, index) {
        ...
        try {
            ...
            if (match = SIMPLE_ANNOTATION_REGEX.exec(line)) return specification.handle('Annotation', { key: match[1].trim(), value: true });
            if (match = NVP_ANNOTATION_REGEX.exec(line)) return specification.handle('Annotation', { key: match[1].trim(), value: match[2].trim() });
