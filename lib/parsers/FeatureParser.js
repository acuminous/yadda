...
var FeatureParser = function(language) {
    ...
    function parse_line(line, index) {
        ...
        var match;
        try {
            specification.line_number = index+1;
            ...
            if (match = TEXT_REGEX.exec(line)) return specification.handle('Text', match[1], line);
        } catch (e) {
...
var Specification = function() {
    ...
    this.handle = function(event, data, line) {
        current_element.line_number=this.line_number;
        current_element = current_element.on(event, data, line);
    };
...
var Examples = function(scenario) {
    ...
    function capture_example(event, data, line) {
        ...
        switch(parsing_mode){
           ...
           case  2: // multi row mode - new row
              ...
              fields.each(function(field, index) {
                ...
                {
                    start_line = _this.line_number;
                    example[headings[index]+".start.line"]=_this.line_number;
                    example[headings[index]+".end.line"]=_this.line_number;
    ...
    this.on = function(event, data, line) {
        return handlers.find(event).handle(event, data, line) || this;
    };
