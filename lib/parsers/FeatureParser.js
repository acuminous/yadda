...
var FeatureParser = function(language) {
    ...
    var SINGLE_LINE_COMMENT_REGEX = new RegExp('^\\s*#', 'i');
    ...
    var create_feature = function(title) {
        ...
        blocks.start(title, write_feature_title);
        return feature;
    };
    ...
    var add_scenario = function(title) {
        ...
        blocks.start(title, write_scenario_title);
    };
    ...
    var write_feature_title = function(lines) {
        feature.title = lines.join('\n');
    };

    var write_scenario_title = function(lines) {
        feature.scenarios[feature.scenarios.length - 1].title = lines.join('\n');
    };
};
...
var Blocks = function() {
    ...
    this.start = function(line, callback) {
        if (this.open()) this.end();
        block = new Block(callback).write(line);
    };
