...
const FeatureParser = function (options) {
  ...
  const FEATURE_REGEX = new RegExp(`^\\s*${language.localise('feature')}:\\s*(.*)`, 'i');
  const RULE_REGEX = language.supports('rule') ? new RegExp(`^\\s*${language.localise('rule')}:\\s*(.*)`, 'i') : /(?!)/; // Never matches when the language has no rule keyword
  const SCENARIO_REGEX = new RegExp(`^\\s*${language.localise('scenario')}:\\s*(.*)`, 'i');
  const BACKGROUND_REGEX = new RegExp(`^\\s*${language.localise('background')}:\\s*(.*)`, 'i');
  const EXAMPLES_REGEX = new RegExp(`^\\s*${language.localise('examples')}:`, 'i');
  const TEXT_REGEX = /^(.*)$/i;
