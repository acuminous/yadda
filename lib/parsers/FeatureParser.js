...
var Scenario = function(title, annotations, feature) {
    ...
    var description = [];
    var steps = [];
    var examples = new Examples(this);
    var handlers = new Handlers({
        text: capture_description,        
        blank: end_description,
...
var Examples = function(scenario) {
    ...
    function capture_example(event, data) {
        ...
        if (fields.length != headings.length) throw new Error('Incorrect number of fields in example table. Expected ' + headings.length + ' but found ' + fields.length);
        var example = {};
    ...
    this.expand = function(scenario) {
        if (examples.length == 0) return scenario;
