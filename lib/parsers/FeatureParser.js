...
function store_annotation_to(annotations, annotation) {
    function store_annotation (key, annotationValue) {
        function store_as_value (key, value) {
            annotations[key] = value;
        }

        function store_as_array (key, value) {
            var existingValue = annotations[key];

            if (!Array.isArray(existingValue)) {
                annotations[key] = existingValue = [ existingValue ];
            }
        ...
        if (!annotations.hasOwnProperty(key)) {
            store_as_value(key, annotationValue);
        } else {
            store_as_array(key, annotationValue);
        }
    }

    var key = annotation.key,
        underscoredKey = annotation.key.toLowerCase().replace(/\W/g, '_');

    store_annotation(key, annotation.value);

    if (key !== underscoredKey) {
        store_annotation(underscoredKey, annotation.value);
    }
}
...
var Specification = function() {
    ...
    function stash_annotation(event, annotation) {
        ...
        store_annotation_to(annotations, annotation);
    };
...
var Feature = function(title, annotations, stashed_annotations) {
    ...
    function stash_annotation(event, annotation) {
        handlers.unregister('background');
        store_annotation_to(stashed_annotations, annotation);
    };
