...
var FeatureParser = function(language) {
    ...
    function parse_line(line, index) {
        ...
        try {
            ...
            if (match = SIMPLE_ANNOTATION_REGEX.exec(line)) return specification.handle('Annotation', { key: trim(match[1]), value: true });
            if (match = NVP_ANNOTATION_REGEX.exec(line)) return specification.handle('Annotation', { key: trim(match[1]), value: trim(match[2]) });
            ...
            if (match = DASH_REGEX.exec(line)) return specification.handle('Dash');
...
var Examples = function(scenario) {
    ...
    function capture_headings(event, data) {
        handlers.register('text', start_example);
        handlers.register('dash', enable_multiline);
        headings = split(data).collect(function(column) {
            return trim(column);
        }).naked();
    }

    function enable_multiline(event, data) {
        handlers.register('text', start_multiline_example)
        handlers.register('dash', end_multiline_example)
    }

    function start_example(event, data) {
        handlers.unregister('dash');
        capture_example(event, data)
    }

    function start_multiline_example(event, data) {
        capture_example(event, data)
        handlers.register('text', capture_multiline_example);
    }
    ...
    function capture_example(event, data) {
        ...
        fields.each(function(field, index) {
            example[headings[index]] = trim(field);
        });
        examples.push(example);
    }

    function capture_multiline_example(event, data) {
        var fields = split(data, headings.length);
    ...
    function end_multiline_example(event, data) {
        handlers.register('text', start_multiline_example);
    }
