...
var FeatureParser = function(language) {
    ...
    var FEATURE_REGEX = new RegExp(
        '^\\s*' + language.localise('feature') + ':\\s*(.*)', 'i');
    var SCENARIO_REGEX = new RegExp(
        '^\\s*' + language.localise('scenario') + ':\\s*(.*)', 'i');
    var BACKGROUND_REGEX = new RegExp(
        '^\\s*' + language.localise('background') + ':\\s*(.*)', 'i');
    var EXAMPLES_REGEX = new RegExp(
        '^\\s*' + language.localise('examples') + ':', 'i');
    ...
    function parse_line(line, index) {
        var match;
        try {
            if (match = MULTI_LINE_COMMENT_REGEX.test(line))
                return comment = !comment;
            if (comment)
                return;
            if (match = SINGLE_LINE_COMMENT_REGEX.test(line))
                return;
            if (match = SIMPLE_ANNOTATION_REGEX.exec(line))
                return specification.handle(
                    'Annotation',
                    {key: match[1], value: true});
            if (match = NVP_ANNOTATION_REGEX.exec(line))
                return specification.handle(
                    'Annotation',
                    {key: match[1], value: match[2]});
            if (match = FEATURE_REGEX.exec(line))
                return specification.handle('Feature', match[1]);
            if (match = SCENARIO_REGEX.exec(line))
                return specification.handle('Scenario', match[1]);
            if (match = BACKGROUND_REGEX.exec(line))
                return specification.handle('Background', match[1]);
            if (match = EXAMPLES_REGEX.exec(line))
                return specification.handle('Examples');
            if (match = BLANK_REGEX.test(line))
                return specification.handle('Blank');
            if (match = TEXT_REGEX.exec(line))
                return specification.handle('Text', match[1]);
        } catch (e) {
            throw new Error(
...
var Handlers = function(handlers) {
    ...
    this.find = function(event) {
        if (!handlers[event.toLowerCase()])
            throw new Error(event + ' is unexpected at this time');
...
var Specification = function() {
    ...
    this.export = function() {
        if (!feature)
            throw new Error('A feature must contain one or more scenarios');
...
var Feature = function(title, annotations, stashed_annotations) {
    ...
    function start_scenario(event, title) {
        var scenario = new Scenario(
            title, background, stashed_annotations, _this);
...
var Background = function(title, feature) {
    ...
    function validate() {
        if (steps.length == 0)
            throw new Error('Background requires one or more steps');
    };
...
var Scenario = function(title, background, annotations, feature) {
    ...
    function validate() {
        if (steps.length == 0)
            throw new Error('Scenario requires one or more steps');
    };
...
var Examples = function(scenario) {
    ...
    function validate() {
        if (headings.length == 0)
            throw new Error('Examples table requires one or more headings');
        if (examples.length == 0)
            throw new Error('Examples table requires one or more rows');
    };
