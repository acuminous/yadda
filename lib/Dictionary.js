...
var Dictionary = function(prefix) {
    ...
    this.expand = function(text, already_expanding) {
        if (!is_expandable(text)) return text;
        return expand_sub_terms(text, $(already_expanding));
    };
    ...
    var expand_sub_term = function(sub_term, already_expanding) {
        var definition = definitions[sub_term] || { pattern: '(.+)' };
        return is_expandable(definition.pattern) ? _this.expand(definition.pattern, already_expanding.concat(sub_term)) : definition.pattern;
    };
