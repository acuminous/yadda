...
var Dictionary = function(prefix) {
    ...
    this.define = function(term, pattern, converters) {
        ...
        if (!is_expandable(pattern) && !converters) converters = $().fill(pass_through_converter, count_matching_groups(pattern));
    ...
    function expand_sub_terms(text, already_expanding) {
        return get_sub_terms(text).each(function(sub_term) {
            ...
            return text = text.replace(prefix + sub_term, sub_term_pattern);
        });
    };
    ...
    function get_converters(text) {
        if (is_expandable(text)) return get_sub_term_converters(text);
    ...
    function get_sub_term_converters(text) {
        return get_sub_terms(text).inject([], function(converters, sub_term) {
            return _this.is_defined(sub_term) ? converters.concat(definitions[sub_term].converters)
                                              : converters.concat(get_converters(expand_sub_term(sub_term, [])))
        });
    }
