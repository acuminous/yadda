...
var Library = function(dictionary) {
    ...
    this.define = function(options, signatures, fn, macro_context) {
        if (!has_options(arguments)) return this.define.apply(this, [{}].concat(slice.call(arguments)))
        $(signatures).each(function(signature) {
            define_macro(options, signature, fn, macro_context);
        });
        return this;
    };

    var has_options = function(args) {
        return typeof args[1] !== 'function' && typeof args[1] !== 'undefined'
    }

    var define_macro = function(options, signature, fn, macro_context) {
