...
const Yadda = function (libraries, interpreter_context) {
  ...
  this.yadda = function (scenario, scenario_context, next) {
    ...
    const steps = scenario ? [].concat(scenario).map(unwrap_step) : scenario;
    this.interpreter.validate(steps);
    this.interpreter.interpret(steps, new Context().merge(interpreter_context).merge(scenario_context), next);
  };
  ...
  // Step-level plugins pass a { name, skip } step object; core matches on the text.
  function unwrap_step(step) {
    return step && typeof step === 'object' ? step.name : step;
  }
};
