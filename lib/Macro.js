...
var Macro = function(signature, parsed_signature, macro, macro_context, library) {
    ...
    this.interpret = function(step, scenario_context, next) {
        ...
        convert(signature_pattern.groups(step), function(err, args) {
            ...
            var macroReturnValue = fn.invoke(macro, context.properties, args.concat(next));
            if (macroReturnValue && macroReturnValue.then) {
                macroReturnValue.then(function() {
                    next();
                });
            };
            if ((!macroReturnValue || !macroReturnValue.then) && macro.length === args.length && next) next();
        });
    };
