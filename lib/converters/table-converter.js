...
module.exports = function table_converter(value, next) {
    ...
    next(null, $(rows).inject([], function(table, row) {
        table.push(parse_fields(row));
        return table;
    }));

    function parse_headings(row) {
        return $(row.split(column_separator)).collect(function(value) {
    ...
    function parse_fields(row) {
        var fields = {};
        $(row.split(column_separator)).each(function(field, index) {
