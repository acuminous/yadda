...
function EventBus() {
  ...
  var notify_handlers = (event_name, event_data) => {
    find_handlers(event_name).each((callback) => {
      callback({ name: event_name, data: event_data });
    });
  };

  var find_handlers = (event_name) => event_handlers.find_all((handler) => new RegExp(handler.pattern).test(event_name)).collect((handler) => handler.callback);
}
